...
Guidance for coding agents (Claude Code, Codex, and other tools) and human contributors working in this repository.

This is the canonical source. `CLAUDE.md` is a symlink to this `AGENTS.md`, so both filenames resolve to one file — edit here and there is no drift.
...
- `uv run anki-api review cards/sample.json --deck "Default"` runs the interactive approval loop against a specific deck; omit `--deck` to keep the decks assigned in each card's JSON.
- `uv run anki-api extract source.docx --output scraped/source.md` converts DOCX references into markdown under `scraped/`; omit `--output` to auto-suffix the filename.
...
- JSON card files should keep `front`, `back`, `context`, `tags`, `deck`, and `model`, plus optional `images` filenames for files stored under `cards/media/`—omit transient review metadata. Keep card filenames kebab-cased.
...
- Name test files `test_<module>.py` so pytest auto-discovers them, and name tests `test_<behavior>`, e.g., `test_validate_atomic_flags_compound_question`, to mirror the EAT rules they safeguard.
- Scope runs with `uv run pytest -k validator` when iterating quickly.
...
Per-repo configuration for the engineering skills (Matt Pocock's skill set). Full details live under `docs/agents/`.

### Issue tracker

Issues are tracked in the repo's GitHub Issues (`SystemicVoid/anki-api`) via the `gh` CLI. See `docs/agents/issue-tracker.md`.
